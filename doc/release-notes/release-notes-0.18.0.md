GleecBTC Core version 0.18.0 is now available from:

  <https://gleecbtccore.org/bin/gleecbtc-core-0.18.0/>
...
`/Applications/GleecBTC-Qt` (on Mac) or `gleecbtcd`/`gleecbtc-qt` (on
Linux).
...
GleecBTC Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use GleecBTC Core on unsupported systems.
...
10.10. Additionally, GleecBTC Core does not yet change appearance when
...
`contrib/init/gleecbtcd.service`, it has been changed to use
`/var/lib/gleecbtcd` as the data directory instead of
`~gleecbtc/.gleecbtc`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/gleecbtcd` will
exist has enough space (check using `df -h /var/lib/gleecbtcd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/gleecbtc/gleecbtc/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://gleecbtccore.org/en/contact/) the GleecBTC Core
...
The systemd init file (`contrib/init/gleecbtcd.service`) has been changed
to use `/var/lib/gleecbtcd` as the data directory instead of
`~gleecbtc/.gleecbtc`. This change makes GleecBTC Core more consistent with
...
`datadir` option in `/etc/gleecbtc/gleecbtc.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/gleecbtc/gleecbtc.conf`.


Documentation
-------------

- A new short [document](https://github.com/gleecbtc/gleecbtc/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/gleecbtc/gleecbtc/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/gleecbtc/gleecbtc/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/gleecbtc/gleecbtc/blob/master/doc/gleecbtc-conf.md)
  about the `gleecbtc.conf` file describes how to use it to configure
  GleecBTC Core.

- A new document introduces GleecBTC Core's BIP174 [Partially-Signed
  GleecBTC Transactions
  (PSBT)](https://github.com/gleecbtc/gleecbtc/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/gleecbtc/gleecbtc/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent GleecBTC-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  GleecBTC Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/gleecbtc/gleecbtc/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/gleecbtc/gleecbtc/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when GleecBTC Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of GleecBTC Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/gleecbtc/gleecbtc/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/gleecbtc/gleecbtc/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart gleecbtcd with the `-deprecatedrpc=generate`
...
- A new `gleecbtc-wallet` tool is now distributed alongside GleecBTC
...
This section describes planned changes to GleecBTC Core that may affect
other GleecBTC software and services.

- Since version 0.16.0, GleecBTC Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with GleecBTC Core 0.20 (expected about
  a year after 0.18), GleecBTC Core will default to native segwit
  ...
  set the `addresstype=bech32` configuration option in any GleecBTC Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, GleecBTC Core
...
- A sub-project of GleecBTC Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with GleecBTC Core.  See their [project
  page](https://github.com/gleecbtc-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to GleecBTC Core's own implementation, although entropy
  gathered by GleecBTC Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves GleecBTC Core a
...
- On macOS, GleecBTC Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `gleecbtc_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libgleecbtc_server.a` (jnewbery)
...
- #13926 [Tools] gleecbtc-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like gleecbtcd (MarcoFalke)
...
- #14521 qt, docs: Fix `gleecbtc-qt -version` output formatting (hebasto)
...
- #11625 Add GleecBTCApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libgleecbtc_server` (laanwj)
...
- #15154 Configure: gleecbtc-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/gleecbtc-wallet (Sjors)
...
- #11526 Visual Studio build configuration for GleecBTC Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libgleecbtc_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in gleecbtc-util-test.py (isghe)
...
- #12775 Integration of property based testing into GleecBTC Core (Christewart)
...
- #12246 Bugfix: Only run gleecbtc-tx tests when gleecbtc-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_gleecbtc` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_gleecbtc` (MarcoFalke)
...
- #14186 gleecbtc-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_gleecbtc.conf` message on startup if conf file exists (leishman)
...
- #12255 Update gleecbtc.service to conform to init.md (dongcarl)
...
- #14497 Add doc/gleecbtc-conf.md (hebasto)
...
- #15354 Add missing `gleecbtc-wallet` tool manpages (MarcoFalke)
...
- #15765 correct gleecbtcconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support gleecbtc folder on OSX (JimmyMow)
